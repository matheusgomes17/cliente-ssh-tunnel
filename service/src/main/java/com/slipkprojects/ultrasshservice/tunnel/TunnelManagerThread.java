...
ServerHostKeyVerifier, DebugLogger
{
			...
			try {
				if (!TunnelUtils.isNetworkOnline(mContext)) {
					...
					mHandler.post(new Runnable() {
							@Override
							public void run() {
								TastyToast.makeText(mContext, "AGUARDANDO REDE...",
	...
	public void stopAll() {
		...
		mHandler.post(new Runnable() {
				@Override
				public void run() {
					TastyToast.makeText(mContext, "DESCONECTADO",
										TastyToast.LENGTH_SHORT,
										TastyToast.ERROR);
				}
			});
	...
	protected void conectar(String servidor, int porta) throws Exception {
		...
		try {
			...
			mHandler.post(new Runnable() {
					@Override
					public void run() {
						TastyToast.makeText(mContext, "CONECTANDO...",
											TastyToast.LENGTH_SHORT,
											TastyToast.DEFAULT);
					}
				});
	...
	protected void autenticar(String usuario, String senha, String keyPath) throws IOException {
		...
		mHandler.post(new Runnable() {
				@Override
				public void run() {
					TastyToast.makeText(mContext, "Autenticando Usuario/Senha",
										TastyToast.LENGTH_SHORT,
										TastyToast.WARNING);
				}
			});
		...
		if (!mConnection.isAuthenticationComplete()) {
			SkStatus.logInfo("Falha ao autenticar, usuário ou senha expirada");
	...
	protected void addProxy(boolean isProteger, int mTunnelType, String mCustomPayload, String mCustomSNI, Connection conn) throws Exception {
		...
		if (mTunnelType != 0) {
			...
			switch (mTunnelType) {
				...
				case Settings.bTUNNEL_TYPE_PAYLOAD_SSL:
					...
					try{
						...
						SSLProxyPayload sslTun = new SSLProxyPayload(sshServer2, sshPort2, customSNI2,customPayload2);
